...
Page({
  ...
  async switchTap() {
    ...
    const res = await deviceControl(device_id, 'switch', !rwDpList.switch.value)
    if (res.success) {
      this.setData({
        'rwDpList.switch.value': !rwDpList.switch.value
      })
    }
    // const res = await deviceControl(device_id, 'switch_led', true)
    console.log(res);
  }
})